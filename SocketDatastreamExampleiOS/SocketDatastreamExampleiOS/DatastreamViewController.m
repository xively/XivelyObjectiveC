...
- (void)updateLabels {
    if (self.datastreamModel) {
        ...
        self.lastUpdatedLabel.text = [NSString stringWithFormat:@"last updated: %@", [self.datastreamModel.info valueForKeyPath:@"at"]];
...
- (void)modelDidFetch:(COSMModel *)model {
    NSLog(@"Did fetch");
    NSLog(@"%@", [Utils describe:self.datastreamModel.info]);
    [self updateLabels];
}
...
- (void)modelFailedToFetch:(COSMModel *)model withError:(NSError *)error json:(id)JSON {
    ...
    [Utils alertUsingJSON:JSON orTitle:@"Failed to fetch feed" message:@"Something went wrong"];
}
